...
/**
 * REST controller for managing {@link com.grt.efreight.domain.ObjectAlias}.
 */
@RestController
@RequestMapping("/object-aliases")
public class ObjectAliasController {

	private final Logger log = LoggerFactory.getLogger(ObjectAliasController.class);

	private static final String ENTITY_NAME = "masterDataManagementServiceObjectAlias";
	 ...
	 *         body the new objectAliasDTO, or with status {@code 400 (Bad Request)}
	 *         if the objectAlias has already an ID.
	 * @throws URISyntaxException if the Location URI syntax is incorrect.
	 */
	@PostMapping
	public ResponseEntity<ObjectAliasDTO> createObjectAlias(@RequestBody ObjectAliasDTO objectAliasDTO)
		...
		return ResponseEntity
				.created(new URI("/api/object-aliases/" + result.getId())).headers(HeaderUtil
	 ...
	 *         the updated objectAliasDTO, or with status {@code 400 (Bad Request)}
	 *         if the objectAliasDTO is not valid, or with status
	 *         {@code 500 (Internal Server Error)} if the objectAliasDTO couldn't be
	 *         updated.
	 * @throws URISyntaxException if the Location URI syntax is incorrect.
	 */
	@PutMapping
	public ResponseEntity<ObjectAliasDTO> updateObjectAlias(@RequestBody ObjectAliasDTO objectAliasDTO)
			throws URISyntaxException {
		...
		return ResponseEntity.ok().headers(HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME,
				objectAliasDTO.getId().toString())).body(result);
	}
	 ...
	 * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body
	 *         the objectAliasDTO, or with status {@code 404 (Not Found)}.
	 */
	@GetMapping("{id}")
