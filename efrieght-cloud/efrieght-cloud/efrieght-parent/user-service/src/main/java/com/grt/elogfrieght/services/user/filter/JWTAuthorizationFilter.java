 ...
 * 
 */
package com.grt.elogfrieght.services.user.filter;
...
import com.grt.elogfrieght.services.user.util.JwtTokenUtil;
...
public class JWTAuthorizationFilter extends BasicAuthenticationFilter {
	...
	protected void doFilterInternal(HttpServletRequest req, HttpServletResponse res, FilterChain chain)
		...
		if (header == null || !header.startsWith("Bearer")) {
			...
			try {
				...
				log.error("authorization exception : {} ", e.getLocalizedMessage());
			}
	...
	private UsernamePasswordAuthenticationToken getAuthentication(HttpServletRequest request)
		...
		if (StringUtils.isNotEmpty(token)) {
			...
			if (user != null) {
				log.info("authService called to get user details : ");
				UserDetailDto userDetails = authService.getUserDetails(user);
	...
	private UsernamePasswordAuthenticationToken createAuthenticationPrinciple(UserDetailDto detailDto) {
		if (detailDto != null) {
			...
			return new UsernamePasswordAuthenticationToken(detailDto.getEmail(), null,
					AuthorityUtils.commaSeparatedStringToAuthorityList(detailDto.getCommmaSeperatedAuthority()));
		}
		return null;
	}
}
