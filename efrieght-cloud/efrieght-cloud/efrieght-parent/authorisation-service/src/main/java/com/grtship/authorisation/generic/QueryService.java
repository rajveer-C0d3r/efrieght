 ...
 * 
 */
package com.grtship.authorisation.generic;
...
import com.grtship.core.filter.RangeFilter;
...
public abstract class QueryService<ENTITY> {
	...
	/**
	 * Helper function to return a specification for filtering on a single field,
	 * where equality, and null/non-null conditions are supported.
	 *
	 ...
	 */
	protected <X> Specification<ENTITY> buildSpecification(Filter<X> filter,
	 ...
	 * where equality, and null/non-null conditions are supported.
	 *
	 * @param filter            the individual attribute filter coming from the
	 *                          frontend.
	 * @param metaclassFunction the function, which navigates from the current
	 *                          entity to a column, for which the filter applies.
	 * @param <X>               The type of the attribute which is filtered.
	 * @return a Specification
	 */
	protected <X> Specification<ENTITY> buildSpecification(Filter<X> filter,
	 ...
	 * {@link java.lang.String} field, where equality, containment, and
	 ...
	 * @return a Specification
	 */
	protected Specification<ENTITY> buildStringSpecification(StringFilter filter,
	 ...
	 * @param filter            the individual attribute filter coming from the
	 *                          frontend.
	 * @param metaclassFunction lambda, which based on a Root&lt;ENTITY&gt; returns
	 *                          Expression - basicaly picks a column
	 * @return a Specification
	 */
	protected Specification<ENTITY> buildSpecification(StringFilter filter,
	 ...
	 * {@link java.lang.Comparable}, where equality, less than, greater than and
	...
	/**
	 * Helper function to return a specification for filtering on one-to-one or
	 * many-to-one reference. Usage:
	 * 
	 * <pre>
	 ...
	 * </pre>
	 *
	 * @param filter     the filter object which contains a value, which needs to
	 *                   match or a flag if nullness is checked.
	 * @param reference  the attribute of the static metamodel for the referring
	 *                   entity.
	 * @param valueField the attribute of the static metamodel of the referred
	 *                   entity, where the equality should be checked.
	...
	protected <OTHER, X> Specification<ENTITY> buildReferringEntitySpecification(Filter<X> filter,
			SingularAttribute<? super ENTITY, OTHER> reference, SingularAttribute<? super OTHER, X> valueField) {
		return buildSpecification(filter, root -> root.get(reference).get(valueField));
	}

	/**
	 * Helper function to return a specification for filtering on one-to-many or
	 * many-to-many reference. Usage:
	 * 
	 * <pre>
	 ...
	 *
	 * @param filter     the filter object which contains a value, which needs to
	 *                   match or a flag if emptiness is checked.
	 * @param reference  the attribute of the static metamodel for the referring
	 *                   entity.
	 * @param valueField the attribute of the static metamodel of the referred
	 *                   entity, where the equality should be checked.
	 ...
	 */
	protected <OTHER, X> Specification<ENTITY> buildReferringEntitySpecification(Filter<X> filter,
			SetAttribute<ENTITY, OTHER> reference, SingularAttribute<OTHER, X> valueField) {
	 ...
	 * </pre>
	 *
	 * @param filter           the filter object which contains a value, which needs
	 *                         to match or a flag if emptiness is checked.
	 * @param functionToEntity the function, which joins he current entity to the
	 *                         entity set, on which the filtering is applied.
	 * @param entityToColumn   the function, which of the static metamodel of the
	 *                         referred entity, where the equality should be
	 *                         checked.
	 * @param <OTHER>          The type of the referenced entity.
	 * @param <MISC>           The type of the entity which is the last before the
	 *                         OTHER in the chain.
	 * @param <X>              The type of the attribute which is filtered.
	 * @return a Specification
	 */
	protected <OTHER, MISC, X> Specification<ENTITY> buildReferringEntitySpecification(Filter<X> filter,
		...
		if (filter.getEquals() != null) {
			...
			// Interestingly, 'functionToEntity' doesn't work, we need the longer lambda
			// formula
	...
	/**
	 * Helper function to return a specification for filtering on one-to-many or
	 * many-to-many reference.Where equality, less than, greater than and
	 * less-than-or-equal-to and greater-than-or-equal-to and null/non-null
	 * conditions are supported. Usage:
	 * 
	 * <pre>
	 * Specification&lt;Employee&gt; specByEmployeeId = buildReferringEntitySpecification(criteria.getEmployeId(),
	 * 		Project_.employees, Employee_.id);
	...
	protected <OTHER, X extends Comparable<? super X>> Specification<ENTITY> buildReferringEntitySpecification(
			final RangeFilter<X> filter, final SetAttribute<ENTITY, OTHER> reference,
	...
	protected <OTHER, MISC, X extends Comparable<? super X>> Specification<ENTITY> buildReferringEntitySpecification(
			final RangeFilter<X> filter, Function<Root<ENTITY>, SetJoin<MISC, OTHER>> functionToEntity,
			Function<SetJoin<MISC, OTHER>, Expression<X>> entityToColumn) {
	...
	/**
	 * Generic method, which based on a Root&lt;ENTITY&gt; returns an Expression
	 * which type is the same as the given 'value' type.
	 *
	 * @param metaclassFunction function which returns the column which is used for
	 *                          filtering.
